...
using System.IO;
using System.Linq;
using ValidationSchedulerData;

namespace SchedulerValidation
{
    public class ScheduleInputParser : IScheduleInputParser
    {
        ...
        private const string ValidatorScheduleFilePath = "ValidationSchedule.csv";
        public IEnumerable<IScheduleRequest> ParseAndCreateScheduleRequest()
        {
            ...
            try
            {
                ...
                foreach(var scheduleItem in validatorScheduleItems)
                {
                    ...
                    if(split.Length != 3)
                    {
                        throw new Exception(string.Format("Incorrect format for schedule input file! - Format is [ValidatorId,ValidatorInstanceId,ScheduledOccurence]{0}    Format Given: {1}",Environment.NewLine,scheduleItem));
                    }
                    ...
                    //TODO - look at nuget package ncrontab to see if it can be used for cron tab validation
